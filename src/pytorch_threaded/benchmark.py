...
def benchmark_mode(
    ...
    with time_range(f"pytorch-threaded run mode {mode_name}", color_id=278):
...
def _threaded_mode_name(mode: ExecutionMode) -> str:
    name = mode.name
    for prefix in ("cupy-naive", "cupy-threaded", "cupy-streams", "pytorch-naive"):
        if name.startswith(prefix):
            return name.replace(prefix, "pytorch-threaded", 1)
    return name
