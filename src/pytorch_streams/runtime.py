...
class DummyGilThread(threading.Thread):
    ...
    def __init__(self, inner_loops: int) -> None:
        super().__init__(name="pytorch-streams-dummy-gil-thread", daemon=True)
...
def start_dummy_gil_thread(
    ...
    with time_range("pytorch-streams start dummy GIL thread", color_id=330):
...
def stop_dummy_gil_thread(
    ...
    with time_range("pytorch-streams stop dummy GIL thread", color_id=331):
