...
DEFAULT_CONFIG_PATH = Path("config_pytorch_streams.yaml")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the single-thread CUDA-stream PyTorch LDH benchmark.",
    )
    parser.add_argument(
        "--config",
        type=Path,
...
def main() -> None:
    ...
    with time_range("pytorch-streams load benchmark config", color_id=331):
    ...
    print(
        f"Single-thread PyTorch stream runtime: num_slots={runtime.num_slots}, "
        f"pipeline_prefetch_batches={runtime.pipeline_prefetch_batches}"
    )

    with time_range("pytorch-streams inspect input", color_id=332):
        print("Inspecting input...")
        info = read_input_info(params.file_path)
        validate_input(info, params)

    with time_range("pytorch-streams preload host data", color_id=333):
    ...
    with time_range("pytorch-streams benchmark suite", color_id=334):
    ...
    with time_range("pytorch-streams write report", color_id=335):
        report_path = write_report(params.report_path, results)
    print(f"Report written to: {report_path}")

    if params.show_image and results:
        with time_range("pytorch-streams show image", color_id=336):
            result = results[-1]
