...
def benchmark_mode(
    ...
    with time_range(f"pytorch-streams init mode {mode.name}", color_id=329):
    ...
    with time_range(f"pytorch-streams run mode {mode.name}", color_id=330):
        runner.prime()
        runner.warmup()
...
def _stream_mode_name(
    ...
    for prefix in (
        ...
        if name.startswith(prefix):
            name = name.replace(prefix, "pytorch-streams", 1)
            break
    else:
        name = f"pytorch-streams | {name}"
